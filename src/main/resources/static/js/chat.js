...
async function fetchConversations() {
    try {
        const response = await fetch('/api-v/conversations');
        if (!response.ok) {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Chat sidebar toggles (mobile)
    const chatSidebar = document.querySelector('.chat-sidebar');
    const toggleButtons = [
        document.getElementById('chatSidebarToggleDefault'),
        document.getElementById('chatSidebarToggleUser'),
        document.getElementById('chatSidebarToggleGroup')
    ].filter(Boolean);
    ...
    document.addEventListener('click', (e) => {
        if (window.innerWidth <= 1024 && chatSidebar && chatSidebar.classList.contains('active')) {
            const isInside = chatSidebar.contains(e.target) || toggleButtons.some(btn => btn.contains(e.target));
            if (!isInside) chatSidebar.classList.remove('active');
        }
    });
});
